...
# Diagnostics (reduced to avoid excessive IO during profiling)
...
# Add a Python callback to demonstrate PInsight cross-layer tracking
step_count = 0
@callfromafterstep
def log_progress():
    global step_count
    step_count += 1
    if step_count % 10 == 0:
        # Accessing pyAMReX native objects via Python
        amr = sim.extension.amr
        nprocs = amr.ParallelDescriptor.NProcs()
        print(f"[Python Control Plane] Completed step {step_count}/{max_steps} across {nprocs} MPI processes.")

print("Starting 3D Laser Wakefield Acceleration (LWFA) Simulation...")
...
with profile(
    ...
    with record_function("sim.step"):
        sim.step(max_steps)

print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
prof.export_chrome_trace("warpx_pytorch_trace.json")
